...
char *mount_paths_default[] = { "/", "/data", NULL };
...
void get_disk_stats(uint64 *sectors_read, uint64 *sectors_written) {
    *sectors_read = *sectors_written = 0;
    FILE *fp = popen("iostat -d", "r");
    if (!fp) {
        return; // iostat not available or failed
    }
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        while (*ptr && isspace(*ptr)) ptr++;
        if (!*ptr) goto next_line;
        *sectors_read += strtoull(ptr, &ptr, 10) * 2;

        while (*ptr && isspace(*ptr)) ptr++;
        if (!*ptr) goto next_line;
        *sectors_written += strtoull(ptr, &ptr, 10) * 2;
...
bool collect(void) {
    ...
    double android_temp = 0.0;
    double android_battery = 0.0;
    if (!cpu_end.total) { // Failed to read /proc/stat
        is_android = true;
        FILE *fp = popen("/data/data/com.termux/files/usr/bin/termux-battery-status", "r");
        if (fp) {
            char line[256];
            while (fgets(line, sizeof(line), fp)) {
                char *p = strstr(line, "\"temperature\":");
                if (p) android_temp = strtod(p + 14, NULL);
                p = strstr(line, "\"percentage\":");
                if (p) android_battery = strtod(p + 13, NULL);
            }
            pclose(fp);
        }
    }
    ...
    if ((!is_android && (!cpu_start.total || !cpu_end.total || // if get_current_jiffies failed
        ...
        (!is_android && (!disk_sectors_read_end || !disk_sectors_written_end || !disk_sectors_read_start || !disk_sectors_written_start)) // if get_disk_stats failed
    ) {
        ...
        if (!is_android && cpu_end.total // if the current get_current_jiffies was successful...
    ...
    if (is_android) {
        DOUBLE_TO_TWO_UINT8(current_stats->cpu_usage, android_temp)
        DOUBLE_TO_TWO_UINT8(current_stats->cpu_iowait, android_battery)
        TWO_UINT8_ZERO(current_stats->cpu_steal);
    } else {
